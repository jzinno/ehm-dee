...
parser.add_argument("-i", "--input", dest = "input", required = True,
                    help = "Input pdb file")
parser.add_argument("-gpu", "--use_gpu", dest = "gpu", default= True,
                    help = "Enables running simulation on GPU")
parser.add_argument("-s", "--solvent", dest = "solvent", default= True,
                    help = "Adds solvent to the simulation")
parser.add_argument("-m", "--minimize", dest = "minimize", default= True,
                    help = "Performs Energy Minimization of the structure before simulation")
parser.add_argument("-t", "--time_step", dest = "time_step", default= 5000,
                    help = "Number of time-steps to perform")
parser.add_argument("-r", "--report_interval", dest = "report_interval", default= 1000,
...
integrator = LangevinMiddleIntegrator(293.15*kelvin, 1/picosecond, 0.004*picoseconds)
...
simulation.reporters.append(StateDataReporter(sys.stdout, report_interval, step=True,
        potentialEnergy=True, temperature=True))

simulation.reporters.append(StateDataReporter(sim_name + '_log.csv', report_interval, step=True,
        potentialEnergy=True, temperature=True))
